...
namespace FPServer {
	...
	public partial class MainForm : Form {
		...
		public void GetIDToolStripMenuItemClick(object sender, EventArgs e) {
			byte[] strID = new byte[256];
			if (0 == (errCode = DeviceWrapper.getDeviceModuleID(strID))) {
				sbrNotes.Text = cmdRes = Encoding.ASCII.GetString(strID);
			}
		}
		public void EnrollToolStripMenuItemClick(object sender, EventArgs e) {
			DeviceWrapper.enroll();
			sbrNotes.Text = cmdRes = "Enroll Ready.";
		}
		public void MatchToolStripMenuItemClick(object sender, EventArgs e) {
			int matchID = -1, matchScore = -1;
			if (0 == (errCode = DeviceWrapper.match(ref matchID, ref matchScore))) {
				sbrNotes.Text = cmdRes = matchID.ToString() + " : " + matchScore.ToString();
			}
		}
		public void DeleteToolStripMenuItemClick(object sender, EventArgs e) {
			if (0 == (errCode = DeviceWrapper.deleteFp(-1))) {
				sbrNotes.Text = cmdRes = "Deleted ALL ID.";
			}
		}
		...
		public void TestLEDToolStripMenuItemClick(object sender, EventArgs e) {
			foreach (int i in new int[] { 1, 3, 4 }) {
				for (int k = 0; k < 8; k++) {
					errCode = DeviceWrapper.setLed(i, k);
					System.Threading.Thread.Sleep(1000);
				}
				sbrNotes.Text = cmdRes = "Test OK. Mode=" + i.ToString();
			}
		...
		private void WebSocketSvr_RecvMsg(object sender, AxWebSocketLite.__WebSocketSvr_RecvMsgEvent e) {
			string fun = e.req.Trim('"') + "ToolStripMenuItemClick";
			MethodInfo mi = this.GetType().GetMethod(fun);
			if (mi == null) return;
			mi.Invoke(this, new object[] { this, EventArgs.Empty });
			WebSocketSvr.SendMsg(e.idxFrom, '"' + cmdRes.Trim('\0') + '"');
		}
