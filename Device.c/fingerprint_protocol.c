...
static const U8Bit s_forhead[8] = {0xF1, 0x1F, 0xE2, 0x2E,
                                   0xB6, 0x6B, 0xA8, 0x8A};
...
static S32Bit FP_protocol_checkout_recv_head_checksum(FP_frame_head_p head) {
  U8Bit checksum =
      FP_protocol_get_checksum((U8Bit *)head, sizeof(FP_frame_head_t) - 1);
...
static S32Bit FP_protocol_checkout_recv_data_checksum(FP_recv_p recv) {
  ...
  checksum = FP_protocol_get_checksum((U8Bit *)recv + sizeof(FP_frame_head_t),
                                      length - 1);
...
static void FP_protocol_get_frame_head(FP_frame_head_p head,
...
static S32Bit FP_protocol_get_complete_send_frame(FP_send_p send,
  ...
  if ((send->cmd_type == cmd_fingerprint) &&
  ...
  /* FP_frame_head_t */
  frame_data_len =
      data_area_len + sizeof(FP_send_t) - sizeof(FP_frame_head_t) + 1;
  ...
  /* check sum */
  p = (U8Bit *)send;
  *(p + sizeof(FP_send_t) + data_area.length) =
      FP_protocol_get_checksum(p + sizeof(FP_frame_head_t), frame_data_len - 1);
...
S32Bit FP_protocol_get_fp_write_ftr_info_frame(FP_send_p send, U16Bit id,
                                               U16Bit length) {
  U8Bit buff[4];
  FP_data_area_t data_field;
...
S32Bit FP_protocol_get_fp_write_ftr_data_frame(FP_send_p send, U16Bit frame,
                                               U8Bit *data, U32Bit length) {
...
S32Bit FP_protocol_get_sys_setled_frame(FP_send_p send,
                                        PFP_LED_CTRL_INFO p_stLEDCtrlInfo) {
  U8Bit buff[5];
  FP_data_area_t data_area;
...
S32Bit FP_protocol_set_sys_getpolicy_frame(FP_send_p send,
                                           U32Bit *p_SysPolicy) {
  U8Bit buff[4];
  FP_data_area_t data_area;
...
S32Bit FP_protocol_fp_auto_enroll_frame(FP_send_p send,
...
S32Bit FP_protocol_send_mesg(FP_send_p send, U32Bit timeout) {
  U32Bit length =
      sizeof(FP_send_t) + FP_protocol_get_data_area_length(send) + 1;
...
S32Bit FP_protocol_recv_complete_frame(FP_recv_p recv, FP_data_area_p data_area,
                                       U32Bit timeout) {
  S32Bit ret = 0;
  ...
  /* FP_frame_head_t */
  ret =
      FP_device_read_data((U8Bit *)recv + sizeof(s_forhead),
  ...
  frame_data_length =
      (U32Bit)(*(U8Bit *)&tmp_len) * 0x100 + *((U8Bit *)&tmp_len + 1);

  /* FP_recv_t and data_area and check sum */
  ret = FP_device_read_data((U8Bit *)recv + sizeof(FP_frame_head_t),
                            frame_data_length, timeout);
  if (FP_OK != ret)
    return ret;
