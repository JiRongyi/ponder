...
typedef __packed struct FP_data_area {
  U8Bit *data;
  U32Bit length;
} FP_data_area_t, *FP_data_area_p;

typedef __packed struct FP_frame_head {
  U8Bit forhead[8];
  U16Bit length;
  U8Bit checksum;
} FP_frame_head_t, *FP_frame_head_p;

typedef __packed struct FP_send_mesg_pkg {
...
typedef __packed struct FP_recv_mesg_pkg {
  FP_frame_head_t frame_head;
  U32Bit pwd;
  U8Bit cmd_type;
  U8Bit cmd_word;
...
typedef enum { sleep_type_normal = 0, sleep_type_deep = 1 } FP_sleep_type;
...
#define COMP_CODE_FINGER_EXTRACT_ERROR                                         \
  (0x09) //指纹提取发生错误，可能原因:图像质量不佳等
#define COMP_CODE_FINGER_MATCH_ERROR                                           \
  (0x0A) //指纹匹配发生错误，可能原因:没有指纹
...
S32Bit FP_protocol_send_delete_mesg(FP_send_p send, S32Bit id_len,
                                    U32Bit timeout);
