...
void FP_action_set_send_buffer(U8Bit *buffer) { s_send_p = (FP_send_p)buffer; }

void FP_action_set_recv_buffer(U8Bit *buffer) { s_recv_p = (FP_recv_p)buffer; }
...
S32Bit FP_action_get_enroll_result(FP_enrollResult_p enrollResult,
                                   U32Bit *errorCode) {
  FP_data_area_t data_area;
  U32Bit ret;
  ...
  enrollResult->fingerID =
      (U16Bit) * (data_area.data + 0) * 0x100 + (U16Bit) * (data_area.data + 1);
...
S32Bit FP_action_get_match_result(FP_matchResult_p matchResult,
  ...
  matchResult->matchScore =
      (U16Bit) * (data_area.data + 2) * 0x100 + (U16Bit) * (data_area.data + 3);
  matchResult->matchID =
      (U16Bit) * (data_area.data + 4) * 0x100 + (U16Bit) * (data_area.data + 5);
  *errorCode = FP_action_get_errorCode(s_recv_p->error_code);
...
S32Bit FP_action_get_SysPolicy(U32Bit *p_SysPolicy, U32Bit *errorCode) {
  ...
  *p_SysPolicy = (data_area.data[0] << 24) | (data_area.data[1] << 16) |
                 (data_area.data[2] << 8) | (data_area.data[3]);

  return FP_OK;
}

S32Bit FP_action_set_SysPolicy(U32Bit *p_SysPolicy, U32Bit *errorCode,
                               U32Bit timeout) {
  FP_data_area_t data_area;
  U32Bit ret;
...
S32Bit FP_action_auto_enroll_recv(FP_enrollResult_p enrollResult,
                                  U32Bit *errorCode, U32Bit timeout) {
  U32Bit ret;
  FP_data_area_t data_area;
  ...
  enrollResult->fingerID =
      (U16Bit) * (data_area.data + 1) * 0x100 + (U16Bit) * (data_area.data + 2);
...
S32Bit FP_action_match_syn_recv(FP_matchResult_p matchResult, U32Bit *errorCode,
                                U32Bit timeout) {
  FP_data_area_t data_area;
  U32Bit ret;
...
S32Bit FP_action_get_ftr_distribution(FP_ftrDistribution_p distribution_p,
                                      U32Bit *errorCode) {
  ...
  ret =
      FP_protocol_recv_complete_frame(s_recv_p, &data_field, DEFAULT_TIME_OUT);
  if (FP_OK != ret)
    return ret;
...
S32Bit FP_action_write_ftr_info(FP_ftrData_p downloadftrData_p,
                                U32Bit *errorCode) {
  FP_data_area_t data_field;
  U32Bit ret;
  ...
  ret = FP_protocol_get_fp_write_ftr_info_frame(s_send_p, downloadftrData_p->id,
                                                downloadftrData_p->length);
  if (FP_OK != ret)
    return ret;
...
S32Bit FP_action_write_ftr_data(U16Bit frame, U8Bit *data, U32Bit length,
                                U32Bit *errorCode, U32Bit timeout) {
  FP_data_area_t data_field;
  U32Bit ret;
