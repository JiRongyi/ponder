...
void getID(void) {
  ...
  if (FP_OK == ret && 0 == errorCode) {
    strcpy_s(strID, moduleID.length, (char*)moduleID.ID);
  }
}
...
void autoEnroll(void) {
  ...
  enrollPara.fingerID_high =
      0xFF; //可修改ID，若 ID 为 0xFFFF 则由系统自动分配指纹 ID 。
  enrollPara.fingerID_low =
      0xFF; //可修改ID，若 ID 为 0xFFFF 则由系统自动分配指纹 ID 。

  enrollPara.enroll_mode =
      0x01; //设置为
            //1则表示按压后需要等待手指抬起再次才可以进行下次注册，设置为0则不需要。
...
GetEnrollResultLabel:
  ret = FP_action_auto_enroll_recv(&EnrollResult, &errorCode,
                                   timeout); //处理返回码
  if (0 != ret) {
  ...
  if (errorCode == COMP_CODE_CMD_DATA_LEN_ERROR ||
      errorCode == COMP_CODE_CMD_DATA_ERROR) {
    //检查命令是否发错，重新开始注册
    goto SendEnrollStartLabel;
  }
...
一般的，分为三种应用场景：
    1、上传模组内正在注册的id：替代保存指纹命令（0x13）+
查询保存结果命令（0x14）， 直接将指纹数据上传至上位机；
...
UploadFtrCheckFtrLabel:
  ...
  /*****************上传模组内存储的指定一个id**********/
  //  Printf("distribution[] = 0x%x\r\n", distribution.distributon[check_id >>
  //  3]);

  if (0 == CHECK_BIT(distribution.distributon[uploadftrData.id >> 3],
                     (uploadftrData.id & 0x7))) {
    // id不存在，重新输出正确id号
    Delay_ms(100);
    goto UploadFtrCheckFtrLabel;
  }
...
UploadFtrGetDataLabel:
  for (; i < frameCount; i++) {
    ...
    if (ret != FP_OK || errorCode != FP_OK) {
      ...
      err_num = 0;
      memcpy(uploadftrData.data + 128 * i, ftrReceiveBuffer,
             128); //模组每帧回复的数据长度其实是固定的128字节
    }
  }
...
DownloadFtrSentDataLabel:
  for (; i < frameCount; i++) {
    ...
    ret = FP_action_write_ftr_data(i, ftrSentBuffer, currentLength, &errorCode,
                                   timeout);
    if (FP_OK != ret || errorCode != FP_OK) {
      err_num++;
      if (err_num < 3) {
...
setSysPolicyLabel:
  ...
  ret = FP_action_set_SysPolicy(&SysPolicy_value, &errorCode,
                                timeout); //处理返回码
  if (FP_OK == ret && 0 == errorCode) {
    //获取策略成功后，模组会自行复位
    Delay_ms(200);
...
int main() {
  FP_device_open("com9", 57600, 0, 8, 0, DEFAULT_TIME_OUT);
  // FP_device_write_data("abcdefg", 8, 500);
  // FP_device_read_data(buf, 8, 500);
  // FP_device_write_data(buf, 8, 500);
  demo_main(11);
  return 0;
}


// DLL_EXPORT int Add(unsigned int A, unsigned int B) { return 2 * (A + B); }
